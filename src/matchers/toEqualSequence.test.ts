...
class Letter {
  ...
  static equals(left: Letter, right: Letter) {
...
(expect as any).addEqualityTesters([Letter.equals]);

function* letterGenerator(name: string): Iterable<Letter> {
...
describe(".toEqualSequence", () => {
  ...
  test.each(arrayInequalityScenarios)(
  ...
  test.each(generatorEqualityScenarios)(
    "passes when given $description",
    ({ actualFactory, expectedFactory }) => {
      const actual = actualFactory();
      const expected = expectedFactory();
  ...
  test.each(generatorInequalityScenarios)(
    "fails when given $description",
    ({ actualFactory, expectedFactory }) => {
      const actual = actualFactory();
      const expected = expectedFactory();
